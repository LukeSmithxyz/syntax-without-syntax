...
\subsection{Uniting Formal Linguistics with Typology and Diachrony}
...
\section{Comparison with other prosodic accounts}

Our account here has been


It is worth it to compare our analysis with other prosodically-related accounts of syntactic phenomena, particularly Contiguity Theory as expounded by \textcite{richards16}.
Richards' model, like ours attempts to account for syntactic differences based on prosodic disparities between languages.
The formal aparatus for this is a general desire for probes and goals to be unified in prosodic domains, a general constraint which \textcite{richards10, richards16} quite elegantly employs to motivate alternations such as \textit{wh}-movement, etc.

Our approach, however, is distinct in that Richards's assumes that sentences are gradually constructed by the \textsc{Merge} operation, and is then gradually attenuated by prosodic demands.
For us, 
