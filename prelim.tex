...
\begin{figure}
	\begin{center}
		\begin{tableau}{cc}
			\inp{$\{S,O,V\}$}	\const{\troc}	\const{\iamb}
			\cand{[S][VO]}	\vio{}	\vio{*!}
			\cand{[SVO]}	\vio{}	\vio{*!}
			\cand{[VSO]}	\vio{*!}	\vio{}
...
%The {\cont} principle represents the desire of the language faculty to map semantic or logical structures onto phonological structure.
%In simple terms, {\cont} will shun any form that incorporates elements into a phonological phrase which are \emph{not} a constituent together.

%In the context of subjects, verbs and objects, this means that two of these constituents are placed in the same phonological phrase, they must be the object and the verb, otherwise {\cont} yields a violation.
...
%To be clear, [VSO] or any other order of elements in one and only one phonological phrase does \emph{not} incur a violation, as the whole phrase is indeed a constituent.
 ...
 %That is, if two phonological constituents are to be under the same phonological phrase, they have a \emph{maximally distinct} contour in stress level.
 %This makes this constraint, in phonological terms, almost an equivalent of {\noclash} at a different level of abstraction.
...
\inp{$\{S,O,V\}$}	\const{\noclash}	\const{\cont}	\const{\topf}	\const{\troc}	\const{\nophi}	\const{\finphi}	\const{\iamb}	\const{\initphi}
...
As a side note, it should be noticed that the constraints {\noclash}, {\cont} and {\topf} are all unviolated and inviolable by any hypothetical intransitive sentence.
...
As a note, notice that, similar to in the case of SVO languages, what allows a [S][OV] parsing as opposed to a monoclausal [SOV] parsing is {\noclash} being more highly ranked than {\nophi}.
If these two constraints are rearranged, we get the constraint structure for a [SOV]/[SV] language.


\begin{figure}
\begin{tableau}{c:c:c:c:c:c|c:c}
	\inp{$\{S,O,V\}$}	\const{\noclash}	\const{\iamb}	\const{\cont}	\const{\initphi}	\const{\topf}	\const{\troc}	\const{\nophi}	\const{\finphi}
